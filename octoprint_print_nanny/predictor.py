...
from print_nanny.utils.visualization import (
...
class ThreadLocalPredictor(threading.local):
    ...
    def percent_intersection(
        self,
        prediction: print_nanny.types.BoundingBoxPrediction,
        area_of_interest: np,
    ) -> float:
        """
    ...
    def postprocess(
        self, image: PIL.Image, prediction: print_nanny.types.BoundingBoxPrediction
    ) -> np.array:
    ...
    def min_score_filter(
        self, prediction: print_nanny.types.BoundingBoxPrediction
    ) -> print_nanny.types.BoundingBoxPrediction:
        ...
        return print_nanny.types.BoundingBoxPrediction(
    ...
    def predict(self, image: PIL.Image) -> print_nanny.types.BoundingBoxPrediction:
...
def explode_prediction_df(
    ts: int, prediction: print_nanny.types.BoundingBoxPrediction
) -> pd.DataFrame:
...
def predict_threadsafe(
    ...
    (ow, oh) = image.size
    original_frame = print_nanny.types.Image(height=oh, width=ow, data=image_bytes)
    ...
    if viz_np is not None:
        ...
        post_frame = print_nanny.types.Image(
            height=vh, width=vw, data=viz_buffer.getvalue()
        )
    else:
        post_frame = None

    return print_nanny.types.MonitoringFrame(
