...
def configure_logger(logger):
    file_logging_handler = CleaningTimedRotatingFileHandler(
        os.path.expanduser("~/.octoprint/logs/plugin_print_nanny.log"),
        when="D",
        backupCount=7,
    )
...
import print_nanny.exceptions
from print_nanny.clients.rest import RestAPIClient, API_CLIENT_EXCEPTIONS
from print_nanny.manager import WorkerManager
from print_nanny.clients.honeycomb import HoneycombTracer
from print_nanny.types import MonitoringModes, PluginEvents, RemoteCommands
...
class OctoPrintNannyPlugin(
    ...
    async def sync_printer_profiles(self, **kwargs):
        ...
        for profile_id, profile in printer_profiles.items():
            ...
            id_map["print_nanny"][created_profile.id] = profile_id
    ...
    async def _write_keypair(self, device):
        ...
        async with aiofiles.open(pubkey_filename, "rb") as f:
            ...
            if hashlib.sha256(content).hexdigest() != device.public_key_checksum:
                raise print_nanny.exceptions.FileIntegrity(
